...
function App() {
  ...
  function handleFilesAdded(added: File[]) {
    ...
    setFiles((current) => [
      ...current,
      ...added.map((file) => ({ id: crypto.randomUUID(), file })),
    ]);
  }
  ...
  const allUploaded =
    files.length > 0 && files.every((f) => statuses[f.id] === "uploaded");
      ...
      <header className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Upload historical timesheet exports to draft a data-backed staffing
            plan.
          </p>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {analyzeMutation.isError && (
                      <p className="text-sm text-red-700">
                        Couldn't start analysis: {analyzeMutation.error.message}
                      </p>
                    )}
                  </div>
                  {analysesRemaining <= 0 && (
                    <p className="text-sm text-amber-700">
                      This token has no analyses remaining.
                    </p>
                  )}
                </div>
              ) : (
